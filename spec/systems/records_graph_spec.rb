...
RSpec.describe "RecordsGraph", type: :system do
  ...
  def month_cal(date)
    if date.strftime("%m").to_i < 7
      # 1/1~6/30
      @from = date.beginning_of_year.to_date.strftime('%Y/%m/%d')
      @to = date.beginning_of_year.since(5.month).end_of_month.to_date.strftime('%Y/%m/%d')
    else
      # 7/1~12/31
      @from = date.end_of_year.ago(5.months).beginning_of_month.to_date.strftime('%Y/%m/%d')
      @to = date.end_of_year.to_date.strftime('%Y/%m/%d')
    end
  end
